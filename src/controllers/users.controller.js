...
export const createUser = asyncHandler(async (req, res) => {
  const { firstName, lastName, email, password, role, subscriptionEnd } = req.body
  if (!firstName || !lastName || !email || !password) {
    throw new ApiError(400, 'Prénom, nom, email et mot de passe sont requis')
  }
  ...
  await pool.query(
    `INSERT INTO users (id, first_name, last_name, email, password_hash, role, status, avatar_color, subscription_end)
     VALUES (?, ?, ?, ?, ?, ?, 'active', ?, ?)`,
    [id, firstName, lastName, email, passwordHash, role === 'admin' ? 'admin' : 'client', avatarColor, subscriptionEnd || null],
  )
...
export const updateUser = asyncHandler(async (req, res) => {
  const { id } = req.params
  const { firstName, lastName, email, status, subscriptionEnd, password, avatarUrl } = req.body
  ...
  if (subscriptionEnd !== undefined) { fields.push('subscription_end = ?'); values.push(subscriptionEnd) }
  if (password) { fields.push('password_hash = ?'); values.push(await bcrypt.hash(password, 10)) }
  if (avatarUrl !== undefined) { fields.push('avatar_url = ?'); values.push(avatarUrl || null) }

  if (fields.length === 0) throw new ApiError(400, 'Aucune modification fournie')

  values.push(id)
  const [result] = await pool.query(`UPDATE users SET ${fields.join(', ')} WHERE id = ?`, values)
  if (result.affectedRows === 0) throw new ApiError(404, 'Utilisateur introuvable')
...
export const getUserStats = asyncHandler(async (req, res) => {
  ...
  const [[plannedRow]] = await pool.query(
    `SELECT
     ...
     FROM planned_sessions WHERE user_id = ?`,
    [id],
  )

  // Semaines : accès + statut (jointure sur toutes les semaines des programmes actifs)
  const [weeksRows] = await pool.query(
    `SELECT
       ...
       uwa.completed_at
     FROM weeks w
     JOIN programs p ON p.id = w.program_id AND p.is_active = 1
     LEFT JOIN user_week_access uwa ON uwa.week_id = w.id AND uwa.user_id = ?
     ORDER BY p.display_order, w.number`,
    [id],
  )

  const weeks = weeksRows.map((r) => {
    // Accès réel : entrée user_week_access prioritaire, sinon semaine 1, sinon !template_locked
    let isUnlocked = false
    if (r.user_unlocked !== null && r.user_unlocked !== undefined) {
    ...
    return {
      ...
      weekTitle: r.week_title || `Semaine ${r.week_number}`,
  ...
  const [progRows] = await pool.query(
    `SELECT
       p.id, p.title, p.type, p.is_locked AS template_locked, p.unlock_after_program_id,
       upa.is_unlocked AS user_unlocked
     FROM programs p
     LEFT JOIN user_program_access upa ON upa.program_id = p.id AND upa.user_id = ?
     WHERE p.is_active = 1
  ...
  for (const r of progRows) {
    ...
    if (r.user_unlocked !== null && r.user_unlocked !== undefined) {
      ...
      const [[done]] = await pool.query(
        ...
        [r.unlock_after_program_id, r.unlock_after_program_id, id],
      )
      isUnlocked = done.total_days > 0 && done.done_days >= done.total_days
    } else {
      isUnlocked = true
    }
    programs.push({
  ...
  // Dernière activité / prochaine planifiée
  const [[lastAct]] = await pool.query(
    `SELECT ended_at, started_at FROM workout_sessions
     WHERE user_id = ? AND status = 'completed'
     ORDER BY COALESCE(ended_at, started_at) DESC LIMIT 1`,
    [id],
  )
  const [[nextPlan]] = await pool.query(
    `SELECT title, scheduled_date, scheduled_time, status FROM planned_sessions
     WHERE user_id = ? AND status = 'planned' AND scheduled_date >= CURDATE()
     ORDER BY scheduled_date, scheduled_time LIMIT 1`,
    [id],
  )

  // Semaine actuelle = plus haute débloquée
  const unlockedWeeks = weeks.filter((w) => w.isUnlocked)
  const currentWeek = unlockedWeeks.length
    ? unlockedWeeks.reduce((a, b) => (a.weekNumber >= b.weekNumber ? a : b))
    : null
  ...
  res.json({
    ...
    completedSessions: Number(sessionsRow.completedSessions) || 0,
    trainingMinutes: Number(sessionsRow.trainingMinutes) || 0,
    activeMinutesEstimate: heartbeatRow.heartbeats,
    plannedCount: Number(plannedRow.planned) || 0,
    plannedDone: Number(plannedRow.done) || 0,
    plannedMissed: Number(plannedRow.missed) || 0,
    weeks,
    programs,
    currentWeek,
    weeksCompleted: weeks.filter((w) => w.isCompleted).length,
    weeksUnlocked: weeks.filter((w) => w.isUnlocked).length,
    lastActivityAt: lastAct?.ended_at || lastAct?.started_at || null,
    nextPlanned: nextPlan
      ? { title: nextPlan.title, date: nextPlan.scheduled_date, time: nextPlan.scheduled_time }
      : null,
  })
})
...
export const getDashboardStats = asyncHandler(async (req, res) => {
  ...
  // Séances terminées (toutes + aujourd'hui)
  const [[sessionStats]] = await pool.query(
    `SELECT
       COUNT(*) AS total,
       SUM(status = 'completed') AS completed,
       SUM(status = 'completed' AND DATE(ended_at) = CURDATE()) AS completed_today
  ...
  const [[plannedStats]] = await pool.query(
    `SELECT
       ...
       SUM(status = 'done') AS done,
       SUM(status = 'missed') AS missed,
       SUM(status = 'planned' AND scheduled_date = CURDATE()) AS planned_today
     FROM planned_sessions`,
  )

  // Détail des séances terminées (récentes) avec client / jour / semaine / programme
  ...
  const completedSessions = completedRows.map((r) => ({
  ...
  for (const c of clients) {
    ...
    // Semaines terminées / débloquées
    const [[weekProg]] = await pool.query(
      `SELECT
         SUM(is_unlocked = 1) AS unlocked,
         SUM(completed_at IS NOT NULL) AS completed_weeks
       FROM user_week_access WHERE user_id = ?`,
      [c.id],
    )
    // Semaine actuelle = plus haute semaine débloquée non terminée, sinon plus haute terminée
    const [[currentWeek]] = await pool.query(
      `SELECT w.number, w.title, p.title AS program_title
       FROM user_week_access uwa
       JOIN weeks w ON w.id = uwa.week_id
       JOIN programs p ON p.id = w.program_id
       WHERE uwa.user_id = ? AND uwa.is_unlocked = 1
    ...
    const [[nextPlan]] = await pool.query(
      `SELECT title, scheduled_date, scheduled_time FROM planned_sessions
       WHERE user_id = ? AND status = 'planned' AND scheduled_date >= CURDATE()
       ORDER BY scheduled_date, scheduled_time LIMIT 1`,
      [c.id],
    )
    // Programmes débloqués / terminés (via user_program_access + isProgramCompleted approximé)
    const [[progAccess]] = await pool.query(
      `SELECT SUM(is_unlocked = 1) AS unlocked FROM user_program_access WHERE user_id = ?`,
      [c.id],
    )

    const completed = Number(sess.completed) || 0
    ...
    clientProgress.push({
      ...
      sessionsTotal: total,
      plannedCount: Number(planned.planned) || 0,
      plannedDone: Number(planned.done) || 0,
      plannedMissed: Number(planned.missed) || 0,
      weeksUnlocked: Number(weekProg.unlocked) || 0,
      weeksCompleted: Number(weekProg.completed_weeks) || 0,
      currentWeekNumber: currentWeek?.number ?? null,
      currentWeekTitle: currentWeek?.title || (currentWeek ? `Semaine ${currentWeek.number}` : null),
      currentProgramTitle: currentWeek?.program_title ?? null,
      lastActivityAt: lastAct?.ended_at || lastAct?.started_at || null,
      nextPlannedTitle: nextPlan?.title ?? null,
      nextPlannedDate: nextPlan?.scheduled_date ?? null,
      programsUnlocked: Number(progAccess.unlocked) || 0,
    })
  }
  ...
  res.json({
    summary: {
      ...
      sessionsCompleted: Number(sessionStats.completed) || 0,
      sessionsCompletedToday: Number(sessionStats.completed_today) || 0,
      planned: Number(plannedStats.planned) || 0,
      plannedDone: Number(plannedStats.done) || 0,
      plannedMissed: Number(plannedStats.missed) || 0,
      plannedToday: Number(plannedStats.planned_today) || 0,
    },
    completedSessions,
    clientProgress,
  })
